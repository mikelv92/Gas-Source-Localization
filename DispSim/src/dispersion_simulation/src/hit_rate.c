...
void hit_rate_init(struct sHitRate *hit_rate,int max_rate_x,int max_rate_y,int max_rate_z, double cell_size) {
	hit_rate->cells_count.x = (int)max_rate_x/cell_size;
	hit_rate->cells_count.y = (int)max_rate_y/cell_size;
	hit_rate->cells_count.z = (int)max_rate_z/cell_size; //3d
	...
	hit_rate->value=arr3dAlloc((int)max_rate_x/cell_size, (int)max_rate_y/cell_size, (int)max_rate_z/cell_size);
	for (z = 0; z < hit_rate->cells_count.z; z++) { //3d
		for (x = 0; x < hit_rate->cells_count.x; x++) {
			for (y = 0; y < hit_rate->cells_count.y; y++) {
				hit_rate->value[x][y][z] = 0.0; //3d
			}		
		}
	}
}
