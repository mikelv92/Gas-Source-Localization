...
#include "simulated_mox/fake_MOX.h"
  ...
  connector[2].scale.z = sensor2;
	...
	while (ros::ok()){
		...
		if (received==0) {
			...
			boost::array<float, 20ul> concentration_read;
			//float concentration_read;
			if (client.call(srv)) {
				//concentration_read=(float)srv.response.odor_r;

				concentration_read = srv.response.odor_r;
				read_msg.data=sensor_response(concentration_read[0], input_gas_type, 0,input_sensor_model,sensor0_log);
				initial = false;
				sensor_read_pub.publish(read_msg);
				ROS_INFO("fake sensor output : %f ", read_msg.data);
				...
				writeConcentration(x_pos, y_pos, z_pos, read_msg.data, sensorIndex);
...
//========================================================================
void  writeConcentration(float pos_xx, float pos_yy, float pos_zz, float concentrationValue, char *sensorIndex)
//========================================================================
{
	static FILE *file;
	...
	//file = fopen("/home/han/gdm_simulator_catkin_ws/src/gas_dispersion_simulation/simulated_mox/src/sensor_log.txt", "a");
	...
	fprintf (file, "%f %f %f %f\n", pos_xx, pos_yy, pos_zz, concentrationValue);
